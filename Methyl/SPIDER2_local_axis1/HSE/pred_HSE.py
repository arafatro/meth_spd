...
def get_input(pssm_file, spider2_file, hhm_file='NULL'):
  ...
  if pssm_aa != spider2_aa:
    ...
    print spider2_aa
    print >>sys.stderr, "ERROR: residue sequence in pssm input and spider2 input are not the same."
    sys.exit() 
  ...
  if hhm_file=='NULL':
    ...
    if pssm_aa != hhm_aa:
      ...
      print hhm_aa
      print >>sys.stderr, "ERROR: residue sequence in hhm input is not the same as that in pssm and spider2 inputs."
      sys.exit() 
...
def main(pssm_file, spider2_file, hhm_file, out_file):
